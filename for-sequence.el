...
\(fn START END [STEP])"
  ...
  (:expander-case
   (`(,id ,(or (and `(,_ ,start-form ,end-form)
    ...
    (cl-with-gensyms (start end step continuep number)
...
\(fn START-OR-END [END [STEP]])"
  ...
  (:expander-case
   (`(,id ,(or (and (or (and `(,_ ,start-or-end-form)
    ...
    (cl-with-gensyms (start-or-end end step start continuep number)
...
See Info node `(for)Sequence Constructors'."
  ...
  (:expander-case
   (`(,id (,_))
    (cl-with-gensyms (current original visited)
